"""
Test script for engagement_ranker.py - processes only 3 creators for testing
"""
...
OUTPUT_FILE = OUTPUT_DIR / "creator_metrics_test.json"
MAX_CREATORS = 3  # Test with just 3 creators
...
def get_creator_folders(input_dir):
    ...
    print(f"Found {len(creator_folders)} creator folders, will test with {MAX_CREATORS}")
    return creator_folders[:MAX_CREATORS]
...
def scrape_creator_profile(username):
    ...
    run_input = {
        "profiles": [f"@{username}"],
        "resultsPerPage": 20,  # Get fewer videos for testing
    ...
    try:
        ...
        max_wait = 180  # 3 minutes max for testing
        wait_time = 0
...
def extract_metrics(items):
    ...
    for item in items:
        video_meta = {
            ...
            "playCount": item.get("videoMeta", {}).get("playCount", 0),
            "diggCount": item.get("diggCount", 0),
            "commentCount": item.get("commentCount", 0),
            "shareCount": item.get("shareCount", 0),
            "text": item.get("text", "")[:100],  # Truncate for testing
...
def main():
    """Main function to collect metrics for test creators."""
    if not API_TOKEN:
    ...
    for i, folder in enumerate(creator_folders, 1):
        ...
        print(f"\n[{i}/{len(creator_folders)}] Testing with @{username}")
    ...
    print(f"Processed: {len(all_metrics)}/{len(creator_folders)} creators")
    ...
    if all_metrics:
        ...
        for username, metrics in all_metrics.items():
            ...
            print(f"  @{username}: {profile['followerCount']:,} followers, {len(metrics['videos'])} videos scraped")
